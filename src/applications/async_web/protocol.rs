...
impl AsyncProtocol for Http {
    fn handle_async_connection(&self, mut stream: AsyncTcpStream) -> AsyncConnectionFuture<'_> {
        Box::pin(async move {
            ...
            let request_line: (Method, String) =
                match Self::parse_request_line(&mut stream, &mut query_params).await {
                    Ok(request) => request,
                    Err(_error) => {
            ...
            let body_params: Value =
                match Self::parse_body(&mut stream, content_length, content_type).await {
                    Ok(Some(body)) => body,
                    Err(_error) => {
                        return stream.write_all(BAD_REQUEST).await;
                    }
            ...
            let result = match self.handlers.get(&(method.clone(), endpoint.clone())) {
                ...
                None => match self.search_wildcard(&method, endpoint.as_str()) {
                    Some(wildcard_handler) => {
                        wildcard_handler.execute(&mut request, &mut response).await
                    }
                    None => {
...
impl Http {
    ...
    async fn parse_header(stream: &mut AsyncTcpStream) -> HashMap<String, String> {
        ...
        loop {
            ...
            let kv: Vec<String> = line_buf.trim().split(':').map(|s| s.to_string()).collect();
    ...
    fn search_wildcard(&self, method: &Method, endpoint: &str) -> Option<&Handler> {
        let mut endpoint_vec = endpoint.split('/').collect::<Vec<&str>>();
